...
const BusManagement = () => {
    ...
    const loadCompanies = async () => {
        try {
            ...
            if (res.success) {
                setCompanies(res.responseObject?.results || res.responseObject || [])
            }
        } catch (e) {
            console.error(e)
        }
    ...
    const loadCars = async () => {
        try {
            ...
            if (res.success) {
                const results = res.responseObject?.results || res.responseObject || []
                const total = res.responseObject?.total || results.length
                setCars(results)
                                ...
                                <CTableBody>
                                    {cars.map((car, idx) => (
                                        <CTableRow key={car.id}>
                                            ...
                                            <CTableDataCell>{companyName(car.company_id)}</CTableDataCell>
