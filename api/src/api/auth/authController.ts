...
class AuthController {
  ...
  public verifyRecoveryInfo: RequestHandler = async (req: Request, res: Response) => {
    const { email, phone } = req.body;
    const response = await authService.verifyRecoveryInfo(email, phone);
    res.status(response.statusCode).send(response);
  };

  public confirmResetPassword: RequestHandler = async (req: Request, res: Response) => {
    const { userId, newPassword } = req.body;
    const response = await authService.confirmResetPassword(userId, newPassword);
    res.status(response.statusCode).send(response);
  };
