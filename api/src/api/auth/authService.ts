...
export class AuthService {
	...
	async register({ email, phone, password, age }: { email: string; phone: string; password: string; age: number }) {
		try {
			...
			if (!Number.isInteger(parsedAge) || parsedAge < 13) {
				return ServiceResponse.failure("Tuổi không hợp lệ", null, StatusCodes.BAD_REQUEST);
			}
			const hashedPassword = await bcrypt.hash(password, 10);
			await this.authRepository.createAsync({ email, phone, password: hashedPassword, age: parsedAge });
	...
	async verifyRecoveryInfo(email: string, phone: string) {
		const user = await this.authRepository.findOne({ email, phone });
		if (!user) {
			return ServiceResponse.failure("Thông tin xác thực không chính xác", null, StatusCodes.BAD_REQUEST);
		}
		...
		return ServiceResponse.success("Xác thực thành công", { userId, token }, StatusCodes.OK);
	}

	async confirmResetPassword(userId: number, newPassword: string) {
		try {
			console.log(userId);
			...
			await this.authRepository.resetPasswordById(userId, hashed);
